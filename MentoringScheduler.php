...
{  
    ...
    public function calculateTimeSlots($start,  $end, $interval = '1 Hour', $mentoring_sessions) {
        ...
        foreach($period as $time) {
            ...
            //Add hour to time
            $_endTime = Carbon::parse($time)->addHour();

            $eligible_date_time = new DateTime();
            $eligible_date_time->modify('+24 hours');

            //Check if end time is after eligible date time
            if($_endTime > $eligible_date_time) {
                $availability = 0;
                $reason = 'Booking is only available 24 hours in advance';
            }
        ...
        //Order times by startTime cronologically
